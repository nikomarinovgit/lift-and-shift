...
# Prompt user to input network variables
  read -p "Enter IP address: (default: $n_ip): " n_ip  
  n_ip=${n_ip:-$n_ip}
  read -p "Enter Network mask: (default: $n_mask): " n_mask
  n_mask=${n_mask:-$n_mask}
  read -p "Enter Gateway: (default: $n_gw): " n_gw
  n_gw=${n_gw:-$n_gw}
  read -p "Enter DNS: (default: $n_dns): " n_dns
  n_dns=${n_dns:-$n_dns}
  read -p "Enter Hostname (default: $n_hostname): " n_hostname
  n_hostname=${n_hostname:-$n_hostname}
...
handle_linux() {
  ...
  # find root lvm
  root_lvm=$(lvs | grep -iP ".*root|.*ubuntu*" | tr -s ' ' | cut -d ' ' -f 3)
  echo -e "\e[32mFound root VG \e[0m$root_lvm"

  # Mount LVM volumes
  for lvm in $root_lvm; do
      lvm_path=$(lvdisplay $root_lvm | grep "LV Path" | grep root | tr -s ' ' | cut -d ' ' -f 4)
...
if ls /mnt/etc/sysconfig/network-scripts/ifcfg-* | tr ' ' '\n' > /dev/null 2>&1 || ls /mnt/etc/NetworkManager/system-connections/ | tr ' ' '\n' > /dev/null 2>&1; then
    ...
    if ls /mnt/etc/sysconfig/network-scripts/ifcfg-* ; then
      ...
      if ls /mnt/etc/NetworkManager/system-connections/* > /dev/null 2>&1; then
          files=$(ls /mnt/etc/NetworkManager/system-connections/* | tr ' ' '\n')
