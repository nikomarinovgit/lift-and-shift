...
[ -e "sda-pt.sf" ] && echo -e "\e[32mChanging volume in Clonezilla backup from sda to vda\e[m" ; cnvt-ocs-dev -b -d /home/partimag $dir_to_restore sda vda || echo -e "\e[32mClonezilla backup is vda so we continue...\e[m" ;
...
echo -e "\e[32mFirst we create extended partition so we can add logical partitions to it :\e[0m"
vda_number=$(($(lsblk -l /dev/vda | grep -v 'lvm' | wc -l) - 1 ))
echo -e "n\ne\n\n\n\nw" | sudo fdisk /dev/vda ;
parts=$(cat parts | tr ' ' '\n' |  grep -E "^sd.{1}$" )
lsblk;
echo -e "\e[32mvda should have vdaX as extended partition\e[m" ;
echo -e "\e[32mparts that are going to be created are $parts\e[m" ;
# read -p "Press Enter to continue..." ; 
