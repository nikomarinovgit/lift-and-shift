...
echo -e "\e[32mWiping /dev/vda with force\e[m" ; vgchange -an ; vgremove -f $(vgdisplay -c | cut -d: -f1) ; wipefs -a /dev/vda -f ; lsblk; echo -e "\e[32mvda should be clean here\e[m" ;
...
#     [ -e "$lvm.tmp" ] && echo -e "\e[32m$lvm.tmp exists\e[0m " || echo -e "\e[31m$lvm.tmp does not exist. Creating it...\e[0m" ; cp $lvm $lvm.tmp ;
...
#         # sed -i "s|$dev_name|$new_dev_name|g" $lvm.tmp ;
