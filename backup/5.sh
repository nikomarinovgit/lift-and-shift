...
if ls /mnt/etc/NetworkManager/system-connections/*.nmconnection > /dev/null 2>&1; then
  ...
  for file in $files; do
      ...
      manual_set=$(cat /mnt/etc/NetworkManager/system-connections/eth0.nmconnection | grep method=manual)
      ...
      if [ -z "$manual_set" ]; then
          ...
          n_if=$(basename $file)
          n_gw=$(cat /mnt/etc/NetworkManager/system-connections/eth0.nmconnection | grep address1 | cut -d '/' -f 2 | cut -d ',' -f 2)
          n_ip=$(cat /mnt/etc/NetworkManager/system-connections/eth0.nmconnection | grep address1 | cut -d '/' -f 1 | cut -d '=' -f 2)
          n_mask=$(cat /mnt/etc/NetworkManager/system-connections/eth0.nmconnection | grep address1 | cut -d '/' -f 2 | cut -d ',' -f 1)
          n_dns=$(cat /mnt/etc/NetworkManager/system-connections/eth0.nmconnection | grep dns | cut -d '=' -f 2 | cut -d ';' -f 1)
