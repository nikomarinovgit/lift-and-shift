...
#  win_net_conf=$(cat reg.dump | grep -E 'IPAddress|DefaultGateway|SubnetMask|NameServer')
  ...
  win_ip=$(cat /tmp/win_net_conf.reg.dump | grep -oP '^"IPAddress.*' | cut -d '=' -f 2)
  win_gw=$(cat /tmp/win_net_conf.reg.dump | grep -oP '^"DefaultGateway.*')
  win_mask=$(cat /tmp/win_net_conf.reg.dump | grep -oP '^"SubnetMask.*')
  win_dns=$(cat /tmp/win_net_conf.reg.dump | grep -oP '^"NameServer.*')
