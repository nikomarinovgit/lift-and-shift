...
windows_auto_net() {
  ...
  # echo -e "$(reged -x /mnt/windows/Windows/System32/config/SYSTEM 'HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters\Interfaces' | grep -E 'IPAddress|DefaultGateway|SubnetMask|NameServer')"

  # echo -e "$(reged -x /mnt/windows/Windows/System32/config/SYSTEM 'HKEY_LOCAL_MACHINE\SYSTEM\ControlSet001\Services\Tcpip\Parameters\Interfaces' | grep -E 'IPAddress|DefaultGateway|SubnetMask|NameServer')"
  # reged -x /mnt/Windows/System32/config/SYSTEM HKEY_LOCAL_MACHINE\SOFTWARE ControlSet001\\Services\\Tcpip\\Parameters\\Interfaces reg.dump

  reged -x /mnt/Windows/System32/config/SYSTEM HKEY_LOCAL_MACHINE\SOFTWARE ControlSet001\\Services\\Tcpip\\Parameters\\Interfaces reg.dump > /dev/null

awk '/^"IPAddress"=hex\(7\)/ {flag=1; print; next} /^"/ {flag=0} flag {print}' reg.dump  > /tmp/win_net_conf_ip.reg.dump
awk '/^"DefaultGateway"=hex\(7\)/ {flag=1; print; next} /^"/ {flag=0} flag {print}' reg.dump > /tmp/win_net_conf_gw.reg.dump
awk '/^"SubnetMask"=hex\(7\)/ {flag=1; print; next} /^"/ {flag=0} flag {print}' reg.dump > /tmp/win_net_conf_mask.reg.dump
# awk '/^"NameServer"=hex\(7\)/ {flag=1; print; next} /^"/ {flag=0} flag {print}' reg.dump >> /tmp/win_net_conf.reg.dump
cat reg.dump | grep -oP '^"NameServer.*' | cut -d '=' -f 2 | cut -d ':' -f 2 |  tr -d '"' > /tmp/win_net_conf-dns.reg.dump

echo $(cat /tmp/win_net_conf_*.reg.dump) | tr -d '\\\n'

# | tr -d '\n\\' | tr -d ' '

 # > /tmp/win_ip.hex

#  win_net_conf=$(cat reg.dump | grep -E 'IPAddress|DefaultGateway|SubnetMask|NameServer')

  # cat reg.dump | grep -E '^"IPAddress|^"DefaultGateway"|^"SubnetMask"|^"NameServer"="[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+"' > /tmp/win_net_conf.reg.dump

#   cat /tmp/win_net_conf.reg.dump

#  awk '/^"IPAddress"=hex\(7\)/ {flag=1; print; next} /^"/ {flag=0} flag {print}' reg.dump > /tmp/win_ip.hex

  # if [ -z "$(cat /tmp/win_net_conf.reg.dump)" ]; then
  #     echo -e "\e[31mNo network configuration found in Windows registry.\e[0m"
  #     return 1
  # fi

  # echo -e "\e[32mNetwork configuration found in Windows registry.\e[0m"

#   win_ip=$(cat /tmp/win_net_conf.reg.dump | grep -oP '^"IPAddress.*' | cut -d '=' -f 2 | cut -d ':' -f 2 | sed 's/...$//' )
#   win_gw=$(cat /tmp/win_net_conf.reg.dump | grep -oP '^"DefaultGateway.*' | cut -d '=' -f 2 | cut -d ':' -f 2 | sed 's/...$//' )
#   win_mask=$(cat /tmp/win_net_conf.reg.dump | grep -oP '^"SubnetMask.*' | cut -d '=' -f 2 | cut -d ':' -f 2 | sed 's/...$//'  )
#   win_dns=$(cat /tmp/win_net_conf.reg.dump | grep -oP '^"NameServer.*' | cut -d '=' -f 2 | cut -d ':' -f 2 |  tr -d '"' )


# win_ip='31,00,39,00,32,00,2e,00,31,00,36,00,38,00,2e,00,39,00,39,00,'

#   win_ip=$(cat /tmp/win_net_conf.reg.dump | grep -oP '^"IPAddress.*' | cut -d '=' -f 2 | cut -d ':' -f 2 | tr -d '\0' )
#   win_gw=$(cat /tmp/win_net_conf.reg.dump | grep -oP '^"DefaultGateway.*' | cut -d '=' -f 2 | cut -d ':' -f 2  )
#   win_mask=$(cat /tmp/win_net_conf.reg.dump | grep -oP '^"SubnetMask.*' | cut -d '=' -f 2 | cut -d ':' -f 2   )
#   win_dns=$(cat /tmp/win_net_conf.reg.dump | grep -oP '^"NameServer.*' | cut -d '=' -f 2 | cut -d ':' -f 2 |  tr -d '"' )



#   echo -e "\e[32mFound Settings : \n \e[0mIP address: $win_ip \n Network mask: $win_mask \n Gateway: $win_gw \n DNS: $win_dns \n "

# hex_to_decimal $win_ip
# echo $decimal_values
# echo $win_ip
# win_ip=$decimal_values
# hex_to_decimal $win_gw
# win_gw=$decimal_values
# hex_to_decimal $win_mask
# win_mask=$decimal_values
# hex_to_decimal $win_dns
# win_dns=$decimal_values

  # echo -e "\e[32mFound Settings : \n \e[0mIP address: $win_ip \n Network mask: $win_mask \n Gateway: $win_gw \n DNS: $win_dns \n "

  # cat reg.dump | grep -E '^"IPAddress|^"DefaultGateway"|^"SubnetMask"|^"NameServer"="[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+"' | tr -s ' ' | cut -d ' ' -f 3
# cat reg.dump | grep -P "^"IPAddress|^"DefaultGateway|^"SubnetMask|^"NameServer"="*.*.*.*" | tr -s ' ' | cut -d ' ' -f 3

# cat reg.dump | grep -E '^"IPAddress|^"DefaultGateway"|^"SubnetMask"|^"NameServer"="[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+"'
...
linux_auto_net_ifcfg () {
    ...
    echo -e "\e[32mNetwork configuration found in \e[0m network-scripts"
